...
const Taskbar: React.FC<TaskbarProps> = ({ openWindows, onWindowClick, onOpenApp, isMobile, isTablet }) => {
  ...
  const startMenuItems: { label: string; emoji: string; appId?: AppId }[] = [
    ...
    { label: "Rehber", emoji: "📇", appId: "contacts" },
    { label: "Radyo", emoji: "📻", appId: "radio" },
    { label: "Seyret", emoji: "📺", appId: "seyret" },
    { label: "Yazeka", emoji: "📝", appId: "yazeka" },
    { label: "Posta Ankara", emoji: "📬", appId: "posta" },
    { label: "Telankara", emoji: "📞", appId: "telankara" },
  ...
  return (
    <>
      ...
      {startOpen && (
        ...
        >
          {!isMobile && (
            <div className="w-6 flex items-end justify-center pb-2" style={{ background: `linear-gradient(to top, ${theme.titleBar}, ${theme.titleBarEnd})` }}>
