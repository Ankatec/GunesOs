...
const BrowserApp: React.FC<{ isMobile?: boolean }> = ({ isMobile }) => {
  const [tabs, setTabs] = useState<Tab[]>([
    { id: "1", title: "Ana Sayfa", url: "ega://anasayfa" },
  ]);
  ...
  const navigate = (url: string) => {
    setTabs((prev) =>
      prev.map((t) =>
        t.id === activeTabId ? { ...t, url, title: getPageTitle(url) } : t
      )
    );
    setAddressInput(url);
  };
  ...
  const renderHomePage = () => (
    <div className="flex flex-col items-center justify-center h-full bg-gradient-to-b from-blue-50 to-white p-4">
      <h1 className={`font-bold text-[#000080] mb-4 ${isMobile ? "text-xl" : "text-2xl"}`}>🌞 EGA Tarayıcı</h1>
      ...
      <div className={`grid gap-3 w-full ${isMobile ? "grid-cols-2 px-2" : "grid-cols-3 max-w-sm"}`}>
        {[
          ...
          <button
            key={item.label}
            className="bg-white border border-gray-200 rounded-lg p-3 text-center text-sm hover:bg-blue-50 cursor-pointer shadow-sm hover:shadow-md transition-shadow"
            onClick={() => {
    ...
    <div className={`p-4 overflow-auto h-full ${isMobile ? "p-2" : ""}`}>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 px-3 py-2 border border-gray-300 rounded text-sm outline-none focus:border-[#000080]"
      ...
      {domainResults.length > 0 && (
        <div className="space-y-2 mb-4">
          {domainResults.map((d, i) => (
            <div key={i} className={`flex items-center justify-between p-3 rounded border ${d.available ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
              <div>
                <span className="font-medium text-sm">{d.name}{d.extension}</span>
      ...
      {cart.length > 0 && (
        <div className="border-t pt-3">
          <h3 className="font-bold text-sm mb-2">🛒 Sepet ({cart.length})</h3>
          {cart.map((d, i) => (
            <div key={i} className="flex justify-between items-center py-1 text-sm">
              <span>{d.name}{d.extension}</span>
  ...
  const renderSearchResults = (query: string) => (
    <div className="p-4">
      <h2 className="text-lg font-bold mb-3">🔍 &quot;{query}&quot; için arama sonuçları</h2>
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="mb-4 border-b pb-3">
          <a href="#" className="text-blue-700 text-sm font-medium hover:underline">
            {query} - Sonuç {i}
          </a>
          <p className="text-[11px] text-green-700">ega://{query.toLowerCase().replace(/\s/g, "-")}-{i}.osmanlı</p>
          <p className="text-xs text-gray-600 mt-1">
            Bu, &quot;{query}&quot; araması için örnek bir sonuçtur. Gerçek içerik yakında eklenecektir.
          </p>
        </div>
      ))}
    </div>
      ...
      <div className="flex items-center bg-[#d0d0d0] px-1 pt-1 gap-[2px] overflow-x-auto">
        {tabs.map((tab) => (
          ...
          >
            ...
            {tabs.length > 1 && (
              <button className="ml-1 text-[10px] hover:text-red-500" onClick={(e) => { e.stopPropagation(); closeTab(tab.id); }}>✕</button>
            )}
          </div>
        ))}
        <button className="w-5 h-5 text-[12px] text-gray-500 hover:text-black rounded flex items-center justify-center shrink-0" onClick={addTab}>+</button>
      </div>

      <form onSubmit={handleAddressSubmit} className="flex items-center gap-1 px-2 py-1 bg-white border-b border-[#a0a0a0]">
        <button type="button" className="text-[14px] px-1 hover:bg-gray-100 rounded shrink-0" onClick={() => navigate("ega://anasayfa")}>🏠</button>
        <button type="button" className="text-[14px] px-1 hover:bg-gray-100 rounded shrink-0" onClick={() => { navigate("ega://alanadi"); setShowDomainPanel(!showDomainPanel); }}>🌍</button>
        <input
          type="text"
          value={addressInput}
          onChange={(e) => setAddressInput(e.target.value)}
          className="flex-1 text-[12px] px-2 py-1 border border-gray-300 rounded bg-gray-50 outline-none focus:border-[#000080] min-w-0"
          spellCheck={false}
        />
        <button type="submit" className="text-[12px] px-2 py-1 bg-[#000080] text-white rounded hover:bg-[#0000a0] shrink-0">Git</button>
      </form>
