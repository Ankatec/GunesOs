...
const Notepad: React.FC<NotepadProps> = ({ onSaveFile }) => {
  const [text, setText] = useState("GüneşOS Not Defteri'ne hoş geldiniz!\n\nBuraya notlarınızı yazabilirsiniz.");
    ...
    <div className="w-full h-full flex flex-col relative">
      ...
      <div className="flex items-center gap-1 px-1 py-[2px] bg-[#e8e8e8] border-b border-[#a0a0a0]">
        <button
          ...
          className="text-[11px] px-2 py-[2px] hover:bg-[#d0d0d0] rounded"
        >
          💾 Kaydet
        </button>
        <button
          onClick={() => { setText(""); setFileName("Adsız.txt"); }}
          className="text-[11px] px-2 py-[2px] hover:bg-[#d0d0d0] rounded"
        >
          📄 Yeni
        </button>
      ...
      {showSaveDialog && (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-10">
          <div className="bg-[#c0c0c0] border-2 border-t-white border-l-white border-b-[#808080] border-r-[#808080] p-4 shadow-lg min-w-[280px]">
            <h3 className="text-sm font-bold mb-3">Farklı Kaydet</h3>
            <div className="mb-3">
              <label className="text-[11px] block mb-1">Dosya Adı:</label>
              <input
                type="text"
                value={fileName}
                onChange={(e) => setFileName(e.target.value)}
                className="w-full px-2 py-1 text-[12px] border border-gray-400 bg-white outline-none"
              />
            </div>
            <div className="mb-3">
              <label className="text-[11px] block mb-1">Konum:</label>
              <span className="text-[11px] text-gray-600">📁 Belgelerim</span>
            </div>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setShowSaveDialog(false)}
                className="px-3 py-1 text-[11px] bg-[#c0c0c0] border border-t-white border-l-white border-b-[#808080] border-r-[#808080] hover:bg-[#d0d0d0]"
              >
                İptal
              </button>
              <button
                onClick={handleSave}
                className="px-3 py-1 text-[11px] bg-[#c0c0c0] border border-t-white border-l-white border-b-[#808080] border-r-[#808080] hover:bg-[#d0d0d0] font-bold"
              >
                Kaydet
              </button>
            </div>
