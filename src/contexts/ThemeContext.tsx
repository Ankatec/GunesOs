...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Migration: ensure newly added default icons (sohbeto, kuran) appear for existing users.
  React.useEffect(() => {
    ...
    if (missing.length > 0) {
      setSettings((prev) => ({
        ...prev,
        visibleIcons: [...prev.visibleIcons, ...missing.filter((id) => !prev.visibleIcons.includes(id))],
      }));
    }
